...
export async function slikRoutes(fastify: FastifyInstance) {
  fastify.post('/api/pengajuan/:id/slik-check', { preHandler: authMiddleware }, async (_req, reply) => {
    throw new ImitraError('BELUM_DIIMPLEMENTASI', 'SLIK check akan diimplementasikan di PR berikutnya', 501);
  });

  fastify.get('/api/pengajuan/:id/slik', { preHandler: authMiddleware }, async (_req, reply) => {
