...
**Tim**: `<!-- ISI: nama tim -->`
**Anggota**: Ray · Eka · Dani · Alfian · Firman · Reffa
**Tanggal**: 2026-08-20
...
| Tidak ada DevOps / Release yang berdiri sendiri | Pekerjaan DevOps menumpuk di Sprint 0 dan Kamis pagi, lalu menipis | **Firman merangkap DevOps / Release** dan menuntaskannya sebelum beban fitur naik |
...
**Penugasan nama di bawah adalah usulan.** Firman menukarnya dalam 5 menit pertama Sprint 0
...
|---|---|---|---|
| **Firman** | Tech Lead / Integrator **+ DevOps / Release** | **FR-01**, **FR-09** | `backend/prisma/` (skema, migrasi, seed), `backend/src/config/`, `src/lib/`, `src/middleware/`, `src/services/status.service.ts`, `src/services/audit.service.ts`, `src/repositories/audit.repo.ts`, `docker-compose.yml`, `.github/`, `AGENTS.md`, `docs/adr/` |
...
**Kenapa FR-13 milik Alfian, bukan Firman**: parameter ada untuk melayani skoring dan margin.
...
(badge status, kartu, panel galat) ada di `main`, Ray dan Eka bekerja dengan tata letak
...
- `backend/prisma/schema.prisma` **hanya** disentuh Firman. Kebutuhan tabel/kolom baru
  disampaikan sebagai permintaan, bukan sebagai PR yang mengubah skema.
- `frontend/src/components/` dan `theme.css` **hanya** disentuh Reffa. Ray dan Eka
...
|---|---|---|---|---|---|
| FR-01 Autentikasi & Otorisasi | P0 | **Firman** | Reffa (S-01) | AC-01, AC-02 | `tests/integration/rbac.spec.ts` |
| FR-02 Pengajuan Mikro | P0 | **Dani** | Ray (S-03) | AC-01 | `tests/integration/pengajuan.spec.ts` |
| FR-03 Upload & Verifikasi Dokumen | P0 | **Dani** | Ray (S-05, S-06) | AC-03 | `tests/integration/dokumen.spec.ts` |
| FR-04 Survei Lapangan | P0 | **Dani** | Ray (S-07) | AC-04 | `tests/integration/skoring-prasyarat.spec.ts` |
...
| FR-09 Audit Trail | P0 | **Firman** | Reffa (S-12) | AC-08, AC-12, AC-13 | `integration/audit.spec.ts`, `audit-readonly.spec.ts` |
| FR-10 Pembiayaan Kelompok | P1 | **Dani** | Ray (S-03) | AC-14 | `integration/kelompok.spec.ts` |
| FR-11 Notifikasi | P1 | Firman (penulisan baris) | **Ray** | kriteria sendiri | `integration/notifikasi.spec.ts` |
...
| docker-compose + CI + migrasi + seed | Infra | **Firman** | — | NFR-01, NFR-09 | dijalankan di CI |
| Kelola Pengguna | Infra | Firman | **Eka** (S-14) | — | — |
...
|---|---|---|
| **Firman** | Repo, undang instruktur + anggota, **branch protection**, isi `AGENTS.md` bagian 2/3/4.1/5.1/7, ADR-0001 | Commit `docs(agents)` **sebelum** commit fitur pertama |
...
| **Dani** | Model data bersama Alfian, lalu draf `schema.prisma` diserahkan ke Firman | Draf skema |
...
| **Ray** | Generate S-03 di Stitch (mobile), siapkan `src/api/` skeleton | Tata letak buat pengajuan |
...
`AGENTS.md` ter-commit, ADR-0001, dan jawaban risiko terbesar — **Firman yang menjawab**,
...
|---|---|
| **Firman** | Migrasi awal + seed idempoten jalan; login (FR-01) + middleware peran; `docker compose up` hidup dari clone bersih; CI hijau |
...
| **Ray** | Form buat pengajuan tersambung, pengajuan tersimpan dan tampil di daftar |
...
|---|---|
| **Firman** | FR-09 audit trail penuh + `REVOKE UPDATE, DELETE` + `GET /api/_routes` (AC-12, AC-13) |
...
| **Reffa** | S-04 detail pengajuan (hub semua tab) + S-12 audit trail; review PR frontend Ray & Eka |
| **Ray** | S-05, S-06, S-07 tersambung penuh termasuk unggah ulang satu dokumen |
...
|---|---|
| **Firman** | Uji `docker compose up` dari clone bersih (**dijalankan Reffa, bukan Firman**), tag `v1.0.0` 15.00, checklist SDD BAB 8.5 |
...
| **Ray** | S-03 varian majelis (total plafon live + level approval) untuk AC-14 |
...
- **Review silang wajib**, pasangannya ditentukan supaya tidak ada PR menganggur:
  ...
  Firman tidak menyetujui PR-nya sendiri. Reffa sebagai QA boleh mereview PR mana pun yang
  menyentuh AC.
- **Distribusi commit dipantau.** Firman menjalankan `git shortlog -sn` setiap gate. Kalau satu
  orang mendekati 35 %, pekerjaan digeser — jangan tunggu sampai 50 % (sanksi −8). Dengan 6
  orang, target sehat adalah 12–22 % per orang.
- **Kalau debat lebih dari 5 menit**, Firman memutuskan dan alasannya dicatat (ADR kalau
...
|---|---|---|---|---|
| **R-1** | **Backend menjadi leher botol** — 3 orang untuk 9 FR P0 + seluruh aturan bisnis, dan satu di antaranya memegang infra | Kamis 14.00: `POST /api/pengajuan` belum menyimpan ke database | Kamis 15.30 (Gate 2) | Reffa pindah ke backend Jumat 09.20 mengambil FR-04 survei dan FR-12 query dashboard. Fondasi UI sudah selesai Kamis 13.00, jadi Ray dan Eka tidak terhambat |
| **R-2** | Tiga orang frontend bertabrakan di berkas bersama | Konflik merge di `components/` atau `theme.css` lebih dari sekali | Segera | Hanya Reffa yang menyentuh `components/` dan `theme.css`. Ray dan Eka meminta varian komponen, tidak membuat tandingan |
...
| **R-4** | Migrasi bertabrakan karena dua orang mengubah skema | `prisma migrate dev` meminta reset di laptop siapa pun | Segera | Hanya Firman yang menyentuh `schema.prisma` |
...
|---|---|---|---|
| 1 | `[FR-01] Autentikasi & otorisasi berbasis peran` | `P0` `fitur` | Firman |
...
| 9 | `[FR-09] Audit trail append-only` | `P0` `fitur` | Firman |
| 10 | `[infra] Mock SLIK sesuai kontrak §6.1 + jalur error 404/503/timeout` | `P0` `fitur` | Alfian |
| 11 | `[infra] docker-compose: db, backend, frontend, mock-slik satu perintah` | `P0` `fitur` | Firman |
| 12 | `[infra] Migrasi awal + seed idempoten (akun, parameter, data demo)` | `P0` `fitur` | Firman |
| 13 | `[infra] Sesuaikan ci.yml ke stack Node/TS + service postgres` | `P0` `fitur` | Firman |
| 14 | `[ui] Fondasi frontend: theme, komponen bersama, layout, router, guard peran` | `P0` `fitur` | Reffa |
| 15 | `[ui] Layar AO mobile-first: buat pengajuan, upload dokumen, survei` | `P0` `fitur` | Ray |
